...
# Required Outcomes
1. Produce one implementation plan file at `ai/orchestra/documents/<branch-name>/plan.md` using the [implementation-plan.template](orchestra.templates/implementation-plan.template.md).
2. At least 1 architecture diagram must be included.
3. Skeletons must be provided for all added or changed modules.
4. Include a concrete [risk](orchestra.snippets/risk.md) assessment grounded in the story, repository evidence and implementation approach.
5. Ensure the risk assessment is validated by all applicable sub agents consulted during planning.
6. Break the implementation into explicit [chunks](orchestra.snippets/chunk.md).
...
2. **Load the template**: Read [implementation-plan.template](orchestra.templates/implementation-plan.template.md) to determine what information is required and the exact section order to follow.
