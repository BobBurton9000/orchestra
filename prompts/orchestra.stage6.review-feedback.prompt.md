...
# Required Outcomes
1. The active or current pull request for `<branch-name>` was identified, or a concrete error was returned if no pull request could be resolved.
...
3. Unresolved reviewer comments were retrieved using GitHub MCP tooling.
4. Every unresolved reviewer comment classified as `Critical` or `High` was either addressed with changes or explicitly reported as blocked with a concrete reason.
5. The judge sub agent confirmed that addressed `Critical` or `High` reviewer comments had been correctly addressed before any commit was pushed.
6. No addressed `Critical` or `High` reviewer comment was left without a GitHub reply describing the changes made.
7. Any code changes made to address reviewer feedback were committed and pushed on the source branch.
8. Reviewer comment replies clearly identified the responder as `Orchestra`.
9. Reviewer comment replies were written using `../templates/reviewer-comment-reply.template.md`.
10. No unrelated or lower-severity review feedback was fixed unless it was required to safely address the targeted comments.
...
# Steps
1. **Load PR and change context**: Resolve the active or current pull request for `<branch-name>` using GitHub MCP tooling. Read `.agents/orchestra/<branch-name>/story.md`, `.agents/orchestra/<branch-name>/plan.md`, and `.agents/orchestra/<branch-name>/execution-report.md` when present to ground the response context.
...
6. **Judge addressed feedback before publication**:
	...
	3. If the judge does not establish that the feedback was correctly addressed, return to step 5 and continue fixing before attempting to commit.
...
8. **Reply to reviewer comments as Orchestra**:
